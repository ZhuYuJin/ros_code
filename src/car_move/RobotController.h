...
class RaspiRobot
{
	private:
		...
		int direction;
		RaspiRobot();
		void setMotors(unsigned char leftGo,unsigned char leftDir,unsigned char rightGo,unsigned char rightDir);
		float getDistance(float minDistance,float maxDistance,int count,int maxLoop);
	
	public:
		static bool init();
		static RaspiRobot *getInstance();
		void forwardByTime(float sec);
		void reverseByTime(float sec);
		void forwardByDistance(int centimeter);
		void reverseByDistance(int centimeter);
		void stop();
		void turnLeftByTime(float sec);
		void turnRightByTime(float sec);
		void turnLeftByDegree(int degree);
		void turnRightByDegree(int degree);
		unsigned char getSwitch1();
		unsigned char getSiwtch2();
		void setLED1(unsigned char state);
		void setLED2(unsigned char state);
